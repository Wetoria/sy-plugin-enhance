<template>
  ...
  >
    ...
    >
      <template #customArea>
        <div class="link-card-container" contenteditable="false" data-editable="false" @mousedown.stop @click.stop>
          <template v-if="getLinkType(item) === 'normal-link'">
            <div class="link-card normal-link-card" @click.stop="openExternalUrl(getLinkUrl(item))" contenteditable="false" data-editable="false">
              <div class="link-content" contenteditable="false" data-editable="false">
                <div class="link-domain" contenteditable="false" data-editable="false">
                  <div class="link-favicon" contenteditable="false" data-editable="false">
                    <img :src="getFaviconUrl(getLinkUrl(item))" alt="favicon" onerror="this.style.display='none'" />
                  </div>
                  <span contenteditable="false" data-editable="false">{{ getDomainName(getLinkUrl(item)) }}</span>
                </div>
                <div class="link-title" contenteditable="false" data-editable="false">{{ linksMetaData[getLinkUrl(item)]?.title || getLinkTitle(item) || getLinkUrl(item) }}</div>
                <div class="link-description" v-if="linksMetaData[getLinkUrl(item)]?.description" contenteditable="false" data-editable="false">
                  {{ linksMetaData[getLinkUrl(item)]?.description }}
                </div>
              </div>
              <div class="link-thumbnail" v-if="linksMetaData[getLinkUrl(item)]?.image" contenteditable="false" data-editable="false">
                <img :src="linksMetaData[getLinkUrl(item)]?.image" alt="thumbnail" />
              </div>
            </div>
          </template>
          <template v-else-if="getLinkType(item) === 'block-ref'">
            <div 
              ...
              data-editable="false"
            >
              <div v-if="blockInfoMap[getBlockRefId(item)]?.loading" class="loading-indicator" contenteditable="false" data-editable="false">
                <a-spin />
              </div>
              <template v-else-if="blockInfoMap[getBlockRefId(item)]?.data">
                <div class="block-ref-content-wrapper" contenteditable="false" data-editable="false">
                  <div class="block-ref-left" contenteditable="false" data-editable="false">
                    <div class="block-ref-icon" v-if="blockInfoMap[getBlockRefId(item)]?.icon" contenteditable="false" data-editable="false">
                      <template v-if="isSingleEmoji(blockInfoMap[getBlockRefId(item)]?.icon)">
                        {{ blockInfoMap[getBlockRefId(item)]?.icon }}
                      </template>
                      <img v-else :src="getAssetUrl(blockInfoMap[getBlockRefId(item)]?.icon)" alt="图标" onerror="this.style.display='none'" />
                    </div>
                    <div class="block-ref-info" contenteditable="false" data-editable="false">
                      <div class="block-ref-title" contenteditable="false" data-editable="false">
                        {{ getBlockTitle(blockInfoMap[getBlockRefId(item)]?.data) }}
                      </div>
                      <div class="block-ref-content" v-if="shouldShowContent(blockInfoMap[getBlockRefId(item)]?.data)" contenteditable="false" data-editable="false">
                        {{ blockInfoMap[getBlockRefId(item)]?.data.content }}
                      </div>
                      <div class="block-ref-alias" v-if="blockInfoMap[getBlockRefId(item)]?.data.alias" contenteditable="false" data-editable="false">
                        {{ blockInfoMap[getBlockRefId(item)]?.data.alias }}
                      </div>
                      <div class="block-ref-memo" v-if="blockInfoMap[getBlockRefId(item)]?.data.memo" contenteditable="false" data-editable="false">
                        {{ blockInfoMap[getBlockRefId(item)]?.data.memo }}
                      </div>
                    </div>
                  </div>
                  <div class="block-ref-banner" v-if="blockInfoMap[getBlockRefId(item)]?.banner" contenteditable="false" data-editable="false">
                    <img 
              ...
              <div v-else class="block-ref-error" contenteditable="false" data-editable="false">
                无法获取块信息
              </div>
            </div>
          </template>
...
import { getBlockAttrs, getBlockByID, request, setBlockAttrs } from '@/api'
...
const processIconCode = (iconCode: string) => {
  ...
  if (iconCode.startsWith('emoji:/')) {
    ...
    // 否则可能是十六进制编码
      ...
      try {
        return String.fromCodePoint(parseInt(emojiPath, 16))
      } catch (e) {
  ...
  if (/^[0-9a-f]+$/i.test(iconCode) && iconCode.length >= 4 && iconCode.length <= 8) {
    try {
      return String.fromCodePoint(parseInt(iconCode, 16))
    } catch (e) {
