...
onMounted(() => {
  ...
  nextTick(() => {
    ...
    // 确保Frame的z-index始终为-1
    const frameEl = document.querySelector(`[data-en-flow-node-id="${flowNode.id}"]`) as HTMLElement
    if (frameEl) {
  ...
  // 监听选中状态变化，确保z-index不变
  watch(() => isSelected.value, (newVal) => {
    nextTick(() => {
      const frameEl = document.querySelector(`[data-en-flow-node-id="${flowNode.id}"]`) as HTMLElement
      if (frameEl) {
        frameEl.style.zIndex = '-1'
